...
import AtollList from '@sikundi/components/lists/AtollList'
        ...
        <Fragment>
            <Container className='grid xl:grid-cols-5 grid-cols-1 xl:gap-4 gap-y-4 py-5 items-start'>
                ...
                <div className='col-span-3 flex flex-col items-end order-1 xl:order-2'>
                    <Button className='bg-secondary px-5 mb-5' loading={loading} onClick={()=>router.push('/primary/atolls/create')}>New Atoll</Button>
                    <AtollList atolls={props.atolls} />
                    {
                        props.totalPages > 1 &&
                        <Paginate url={'/primary/atolls?page='} current={props.current} total={props.totalPages} />
                    }
                </div>
            </Container>
        </Fragment>
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
    ...
    const totalAtolls = await prisma.atolls.aggregate({
        _count: true
    })

    if (atolls.length === 0 && ctx.query.page !== undefined) {
        return {
            props: {

            },
            redirect: {
                destination: '/primary/atolls',
                permanent: false
            }
        }
    }
    ...
    return {
        props: {
            atolls:  JSON.parse(JSON.stringify(atolls)),
            totalPages: Math.ceil((Number(totalAtolls._count)/per_page)),

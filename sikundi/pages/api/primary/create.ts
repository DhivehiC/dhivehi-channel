        ...
        try {
            ...
            jwt.verify(String(token), String(secret), async (err:any, user:any)=>{
                if (err) {
                    ...
                    try {
                        const voteBox:any = await create({
                        ...
                        res.status(200).json({ voteBox, notification: { title: "Vote Box Created", content: `${voteBox.name} has been successfully created in the cms`, type: "success" } })
                    } catch (error) {
    ...
    return new Promise(async (resolve, reject)=>{
        try {
            ...
            if (Object.keys(validationErrors).length) {
                ...
                const voteBox = await prisma.voteBoxes.create({
                    data: {

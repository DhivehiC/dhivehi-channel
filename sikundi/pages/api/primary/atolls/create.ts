        ...
        try {
            ...
            jwt.verify(String(token), String(secret), async (err:any, user:any)=>{
                if (err) {
                    ...
                    try {
                        const atoll:any = await create({
                            name: req.body.name,
                        })
                        res.status(200).json({ atoll, notification: { title: "atoll Created", content: `${atoll.name} has been successfully drafted in the cms`, type: "success" } })
                    } catch (error) {
    ...
    return new Promise(async (resolve, reject)=>{
        try {
            ...
            if (Object.keys(validationErrors).length) {
                ...
                const atoll = await prisma.atolls.create({
                    data: {
                        name: inputs.name,
                    },
                    select: {

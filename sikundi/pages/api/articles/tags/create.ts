        ...
        try {
            ...
            jwt.verify(String(token), String(secret), async (err:any, user:any)=>{
                if (err) {
                    ...
                    try {
                        const tag:any = await create({
                            title: req.body.title,
                            latin_title: req.body.latin_title,
                            email: user.email
                        })
                        res.status(200).json({ tag, notification: { title: "Tag Created", content: `${tag.title} has been successfully drafted in the cms`, type: "success" } })
                    } catch (error) {
